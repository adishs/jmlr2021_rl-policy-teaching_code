...
class teaching:
    def __init__(self, env, target_pi, epsilon, p, delta, costr, costp, d_0,
                 T_Q_learning, alpha_Q_learning, attackers_cost_y_axis,
                 epsilon_Q_learning, teacher_type):
        self.env = env
        self.reward_no_attack = env.reward
        _, _, self.pi_no_attack, _ = MDPSolver.valueIteration(env, env.reward)
        ...
        self.costp = costp
        self.d_0 = d_0
        self.T_Q_learning = T_Q_learning
        self.attackers_cost_y_axis = attackers_cost_y_axis
        self.alpha_Q_learning = alpha_Q_learning
        self.epsilon_Q_learning = epsilon_Q_learning
        self.teacher_type = teacher_type

        self.M_0 = env.get_M_0()
        self.pool = teacher.generate_pool(self.M_0[0], self.M_0[1], self.M_0[2], self.M_0[3], M_0[4], self.d_0,
                             target_pi)
    ...
    def end_to_end_teaching(self):
        ...
        print("here")
        self.learner.q_learning(n_rounds=self.T_Q_learning,
                          pi_no_attack=self.pi_no_attack,
                          M_0=M_0, M_t=M_t, cost_y_axis=self.attackers_cost_y_axis,
                          costr=self.costr, costp=self.costp, alpha=self.alpha_Q_learning,
                                epsilon=self.epsilon_Q_learning)
    #enddef
...
if __name__ == "__main__":
    ...
    for iter_num in range(1, number_of_iterations + 1):
        ...
        d_0 = np.zeros(M_0[0])
        d_0[0] = 1
        epsilon_Q_learning = 0.001
        T_Q_learning = (1e+5)*5 + 1



        ################## General Joint p=inf #########################
        teaching_obj_general_reward = teaching(env, target_pi=target_pi, epsilon=0.1, p=np.inf, delta=0.0001,
                                               costr=costr, costp=costp, d_0=d_0, T_Q_learning=T_Q_learning,
                                               alpha_Q_learning=0.5, attackers_cost_y_axis=1,
                                               epsilon_Q_learning=epsilon_Q_learning, teacher_type="general_attack_joint")
        print("general_attack_joint_p=inf")
        ...
        teaching_obj_general_reward = teaching(env, target_pi=target_pi, epsilon=0.1, p=np.inf, delta=0.0001,
                                               costr=costr, costp=costp, d_0=d_0, T_Q_learning=T_Q_learning,
                                               alpha_Q_learning=0.5, attackers_cost_y_axis=1,
                                               epsilon_Q_learning=epsilon_Q_learning, teacher_type="non_target_attack_joint")
        print("non_target_attack_joint_p=inf")
        teaching_obj_general_reward.end_to_end_teaching()
        accumulator_dict = accumulator_function(teaching_obj_general_reward.learner, accumulator_dict,
                                                teaching_obj_general_reward.teacher_type, p="inf")
