...
class teaaching:
    ...
    def offline_attack(self):
        for setting in self.settings_to_run:
            ...
            d_0 = self.teachers_to_run[0][2]["d_0"]
            R_c = M_0[2][0][0]
            # print(R_c)
            # exit(0)
            ...
            pool = teacher.generate_pool(M_in[0], M_in[1], M_in[2], M_in[3], M_in[4], d_0, self.target_pi)
            for tchr in self.teachers_to_run:
                ...
                _, _, pi_T, _ = MDPSolver.valueIteration(env_out, env_out.reward)
                print(teacher_type)
...
if __name__ == "__main__":
    ...
    plot_chain.plot_offline_teaching_vary_eps(dict_file=dict_accumulator, each_number=1, show_plots=False)

        # print(acc_dict)
        # write_into_file(acc_dict, exp_iter=iter_num, teacher_type="offline_teaching_vary_eps_costr={}_costp={}".format(costr, costp))
    exit(0)
