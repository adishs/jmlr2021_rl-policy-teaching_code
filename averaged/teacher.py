...
import MDPSolver
sys.path.append('../src')
from code_attacker_average.reward_attack_general import *
from code_attacker_average.reward_attack_nontargetonly import *
from code_attacker_average.dynamic_attack_nontargetonly import *
from code_attacker_average.joint_attack_nontargetonly import *
...
def create_perturb_P_for_target(num_states, num_actions, R, P_in, gamma, pi, alpha, beta, N):
...
def generate_pool(num_states, num_actions, R, P_in, gamma, pi, alpha=0.1, beta=0.0001, n_copies_of_N=5):
    N_array = [0, 10, 20, 30, 40, 50, 60, 80, 100, 150, 200, 250, 300]
    pool = []
    for n_copy in range(n_copies_of_N):
        for N in N_array:
            P_out = create_perturb_P_for_target(num_states, num_actions, R, P_in, gamma, pi, alpha, beta, N)
            pool.append(P_out)
    return pool
#enddef
